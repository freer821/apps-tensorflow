...
class EmoRecognition:
    ...
    def createCNNNetwork(self):
        ...
        network = self.create_fc_layer(input=network,
                                             num_inputs= 128,
                                             num_outputs=len(self._classes))

        # build predicts
        prd_ary = tf.nn.softmax(network,name='emo_prd')
    ...
    def createCNNNetworkWithSlim(self):
        with slim.arg_scope([slim.conv2d, slim.fully_connected],
            ...
            net = slim.conv2d(self._images, 64, [5, 5], scope='conv1')
    ...
    def train(self, num_iteration, dataTrainBatch, dataValBatch):
        ...
        for i in range(num_iteration):
            ...
            if i % 20 == 0:
                ...
                saver.save(self._session, save_path = self._model_dir+'/emotion-model')
                tf.train.write_graph(self._session.graph_def, self._model_dir, 'emotion-model.pbtxt')
